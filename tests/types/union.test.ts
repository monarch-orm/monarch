...
describe("Union Types", () => {
  describe("union", () => {
    test("union", () => {
      ...
      // @ts-expect-error
      expect(() => Schema.input(schema, { emailOrPhone: {} })).toThrow("no matching variant found for union type");
      // @ts-expect-error
      expect(() => Schema.input(schema, { emailOrPhone: [] })).toThrow("no matching variant found for union type");
      // @ts-expect-error
      expect(() => Schema.input(schema, { emailOrPhone: null })).toThrow("no matching variant found for union type");
  ...
  describe("taggedUnion", () => {
    test("taggedUnion", () => {
      ...
      // @ts-expect-error
      expect(() => Schema.input(schema, { color: {} })).toThrow("missing field");
      // @ts-expect-error
      expect(() => Schema.input(schema, { color: { tag: "hex" } })).toThrow("missing field 'value' in tagged union");
      expect(() =>
        // @ts-expect-error
        Schema.input(schema, { color: { value: "#fff" } }),
      ).toThrow("missing field 'tag' in tagged union");
      expect(() =>
        Schema.input(schema, {
          // @ts-expect-error
          color: { tag: "hex", value: "#fff", extra: "user" },
        }),
      ).toThrow("unknown field 'extra', tagged union may only specify 'tag' and 'value' fields");
      expect(() =>
        // @ts-expect-error
        Schema.input(schema, { color: { tag: "hwb", value: "#fff" } }),
      ).toThrow("unknown tag 'hwb'");
      expect(() =>
        // @ts-expect-error
        Schema.input(schema, { color: { tag: "hsl", value: "#fff" } }),
      ).toThrow("invalid value for tag 'hsl'");
