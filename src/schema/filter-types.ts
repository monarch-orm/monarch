...
type MapKVUpdateEnhanced<T extends KV<any, any>, Target = AnyMonarchType, Value = never> = {
  [K in T["key"] as TargetKey<T, K, Target>]?: IsNever<Value> extends true
...
type MapKVUpdateBit<T extends KV<any, any>> = {
  [K in T["key"] as TargetKey<T, K, IntegerType>]?: BitValue<InferTypeOutput<Extract<T, { key: K }>["value"]>>;
};
type MapKVUpdateAddToSet<T extends KV<any, any>> = {
  [K in T["key"] as TargetKey<T, K, MonarchArray<any>>]?:
...
type MapKVUpdatePullAll<T extends KV<any, any>> = {
  [K in T["key"] as TargetKey<T, K, MonarchArray<any>>]?: InferTypeInput<Extract<T, { key: K }>["value"]>;
};
...
type UpdateOperators<T extends KV<any, any>> = {
  $currentDate?: Pretty<MapKVUpdateEnhanced<T, MonarchDate, true | { $type: "date" }>>;
  $inc?: Pretty<MapKVUpdateEnhanced<T, NumericType>>;
  $min?: Pretty<MapKVUpdate<T>>;
  $max?: Pretty<MapKVUpdate<T>>;
  $mul?: Pretty<MapKVUpdateEnhanced<T, NumericType>>;
  $rename?: Pretty<MapKVUpdateRename<T>>;
  $set?: Pretty<MapKVUpdate<T>>;
  $setOnInsert?: Pretty<MapKVUpdate<T>>;
  $unset?: Pretty<MapKVUpdateEnhanced<T, MonarchOptional<any>, "" | true | 1>>;
  $addToSet?: Pretty<MapKVUpdateAddToSet<T>>;
  $pop?: Pretty<MapKVUpdateEnhanced<T, MonarchArray<any>, 1 | -1>>;
  $pull?: Pretty<MapKVUpdatePull<T>>;
  $push?: Pretty<MapKVUpdatePush<T>>;
  $pullAll?: Pretty<MapKVUpdatePullAll<T>>;
  $bit?: Pretty<MapKVUpdateBit<T>>;
};
