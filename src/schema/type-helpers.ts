...
export type IndexKey<T extends AnyMonarchType, P extends string> =
  Unwrap<T> extends MonarchObject<infer U>
    ? { [K in keyof U]: IndexKey<U[K], Prefix<P, K & string>> }[keyof U] | Prefix<P, "$**"> | P
    : Unwrap<T> extends MonarchRecord<any>
      ? Prefix<P, "$**"> | P
      : Unwrap<T> extends MonarchArray<infer U>
        ? IndexKey<U, P> | P
        : Unwrap<T> extends MonarchTuple<infer U>
          ? { [K in keyof U & `${number}`]: IndexKey<U[K], Prefix<P, K>> }[keyof U & `${number}`] | P
          : Unwrap<T> extends MonarchMixed
            ? Prefix<P, "$**"> | P
            : P;
export type CreateIndexKey<T extends Record<string, AnyMonarchType>> = {
  [K in keyof T as IndexKey<T[K], K & string>]?: 1 | -1 | Exclude<IndexDirection, number>;
};
 ...
 * - `$set` / `$min` / `$max` — all fields
 * - `$setOnInsert`  - full document
 */
export type UpdateFilter<T extends AnySchema> = UpdateOperators<
  T,
  UpdateFilterKV<MonarchObject<InferSchemaTypes<T>>, "">
>;
...
type UpdateOperators<TSchema extends AnySchema, T extends KV<any, any>> = {
