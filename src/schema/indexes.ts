...
export type SchemaIndexes<TTypes extends Record<string, AnyMonarchType>> = (options: {
...
export type CreateIndexesFields<T extends Record<string, AnyMonarchType>> = {
  [K in IndexKeys<_InferTypeObjectOutput<T>> | "_id"]?: 1 | -1 | Exclude<IndexDirection, number>;
};
...
type IndexKeys<T, Prefix extends string = ""> =
    ...
    : T extends ObjectId
      ...
      : T extends Record<string, any>
        ? keyof T extends infer K extends string
          ...
          : never
        : never;
type SubKeys<T, K, Prefix extends string> = K extends keyof T & string ? IndexKeys<T[K], `${Prefix}${K}.`> : never;
type KeysWithWildcard<K extends string, Prefix extends string> = string extends K
  ? `${Prefix}$**`
  : `${Prefix}${K}` | `${Prefix}$**`;
